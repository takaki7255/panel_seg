"""
Training script for UNetGrayLSDSDF (Gray + LSD + SDF input U-Net)

Requires preprocessed dataset with LSD and SDF maps.
Run preprocess_lsd_sdf.py first.
...
class PanelDatasetLSDSDF(Dataset):
    """Dataset for Gray + LSD + SDF input"""
...
def parse_args():
    parser = argparse.ArgumentParser(description='UNetGrayLSDSDF Training')
...
def main():
    ...
    print("\n" + "="*60)
    print("UNetGrayLSDSDF Training (Gray + LSD + SDF)")
    print("="*60)
    print(f"Device: {dev}")
    ...
    prefix = f"{dataset_name}-unetgraylsdsdf"
    ...
    model = UNetGrayLSDSDF(in_channels=3, n_classes=1, base_channels=cfg["BASE_CHANNELS"]).to(dev)
