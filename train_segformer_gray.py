...
Usage:
    ...
    # Resume training
    python train_segformer_gray.py --resume ./panel_models/panel_seg-segformergray-b2-01.pt
"""
...
CFG = {
    ...
    # Wandb
    "WANDB_PROJ": "panel-seg-segformer-gray",
    "DATASET": "panel_seg",
    "RUN_NAME": "",
...
def emergency_save_and_exit(model, cfg, epoch, reason):
    print(f"\n🚨 Emergency: {reason}")
    torch.save(model.state_dict(), f"emergency_segformer_gray_epoch{epoch}.pth")
    sys.exit(0)
...
# ============================================================================
def parse_args():
    parser = argparse.ArgumentParser(description='SegFormerGray Training')
...
def main():
    ...
    print("\n" + "="*60)
    print("SegFormerGray Training (Grayscale only)")
    print("="*60)
    print(f"Device: {dev}")
    ...
    prefix = f"{dataset_name}-segformergray-{model_short}"
